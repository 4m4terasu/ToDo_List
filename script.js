...
function toggleTheme() {
  ...
  localStorage.setItem('theme', isNightMode ? 'night-mode' : 'light-mode'); // Save the theme to localStorage
}

function loadTasks() {
  const tasks =
    JSON.parse(localStorage.getItem(`tasks_${currentCategory}`)) || [];
...
// Call loadCategories and loadTheme on DOMContentLoaded
...
function loadTheme() {
  ...
  if (savedTheme === "night-mode") {
    ...
    document.querySelector(".todo-container").classList.add("night-mode");
  }
}
